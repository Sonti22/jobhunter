...
_PATTERNS = [
    ...
    (REJECTION, 0.9, r"(к сожалению|не готовы продолж|отказ|не подход|"
                     ...
                     # Живые ответы из базы (проверка 19.09), оставшиеся «не понял»:
                     # «Уже закрыли позицию», «вакансию захолдили», «уже выбрал кандидата».
                     r"закрыли\s+(?:эту\s+)?(?:позици|ваканси)\w*|"
                     r"(?:позици|ваканси)\w+\s+(?:уже\s+)?закрыли|"
                     r"захолд\w+|на\s+холде|\bon\s+hold\b|"
                     r"(?:уже\s+)?выбрал\w*\s+(?:другого\s+)?кандидат\w*|"
                     # Написали соискателю, приняв его резюме за вакансию: «это не
                     # вакансия, это моё резюме», «я работу ищу, а не сотрудника».
    ...
    (TECH_QUESTION, 0.85, r"(расскажите,?\s+как|как\s+бы\s+вы|"
                          ...
                          r"работали\s+ли\s+вы\s+с|есть\s+(?:ли\s+)?(?:у\s+вас\s+)?опыт|"
                          # Ответы — в faq профиля (24.09): последний проект, выход.
                          r"расскажите\s+(?:\w+\s+){0,2}(?:о|про)\s+(?:\w+\s+){0,2}проект\w*|"
                          r"tell\s+(?:me|us)\s+about\s+(?:your\s+)?(?:\w+\s+)?project|"
                          r"когда\s+(?:\w+\s+)?(?:сможете|можете|готовы)\s+"
                          r"(?:выйти|приступить|начать)|срок\w*\s+выхода|"
                          r"when\s+(?:can|could)\s+you\s+start|notice\s+period|"
    ...
    (WORK_FORMAT, 0.85, r"(готов\w*\s+(?:ли\s+)?(?:вы\s+)?(?:к\s+)?"
                        ...
                        # «Готовы к офису?», «Готовы ли к посещению офиса в Москве?»
                        r"готов\w*\s+(?:ли\s+)?(?:вы\s+)?(?:к\s+)?(?:посещени\w+\s+|работе\s+в\s+)?офис\w*|"
                        r"как\w*\s+формат\w*\s+работы|формат\w*\s+работы\s+(?:рассматрива|интересу|подход)\w*|"
                        r"(?:удал[её]нк|удал[её]нн\w+\s+формат|гибрид|офис)\w*\s+"
                        r"(?:устраивает|устроит|подходит|подойд[её]т|интересует|рассматриваете)|"
                        r"(?:устраивает|устроит|подходит|подойд[её]т)\s+(?:ли\s+)?(?:вам\s+)?"
                        r"(?:удал[её]нк|гибрид|офис)\w*|"
                        r"where\s+are\s+you\s+(?:based|located)|"
                        r"is\s+(?:a\s+)?remote\s+(?:ok|okay|fine|possible)|"
                        r"open\s+to\s+(?:hybrid|office|on-?site)|"
    ...
    (ASK_CV, 0.9, r"((?:пришлите|присыла\w+|присыл|скиньте|отправьте|вышлите|скинь|пришли|"
                  r"поделитесь|шарьте|направьте|сбросьте|киньте|"
                  # неопределённая форма: «можете скинуть резюме», «просьба прислать CV»
                  r"скинуть|прислать|отправить|выслать|направить|сбросить|кинуть)[^.!?\n]{0,40}"
                  r"(?:резюме|cv\b|си-?ви)|"
                  # «могу запросить резюме?», «могу попросить вас резюме?» (проверка 19.09)
                  r"(?:запросить|попросить|получить|посмотреть|увидеть)[^.!?\n]{0,25}резюме|"
