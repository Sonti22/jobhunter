...
def prompt_message(role: str, jd_excerpt: str, facts: str, allowed: str,
    ...
    lang_rule = ("- весь текст сообщения — СТРОГО ПО-АНГЛИЙСКИ, ни одного "
                 ...
                 "remote-only (any country), English C1\n"
                 if lang == "en" else "")
...
{quote_rule}
- предпоследняя: ОБЯЗАТЕЛЬНО скажи, что кандидат рассматривает ТОЛЬКО
  удалённый формат (страна не важна) и владеет английским на уровне C1.
  Это жёсткое условие кандидата, без него письмо бессмысленно.
