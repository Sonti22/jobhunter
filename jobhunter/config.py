...
class Settings(BaseSettings):
    ...
    # Внешний сторож (healthchecks.io, бесплатно): автопилот отмечается по этому адресу раз в
    # 5 минут. Наш собственный сторож живёт на той же машине и молчит, когда она выключена;
    # внешний замечает именно это. Адрес содержит секретный идентификатор — в логи не пишем.
    ...
    # Tavily Search API: 1000 запросов в месяц бесплатно, без карты. Страницы вне сайта
    # компании, где опубликован адрес руководителя. Пусто — источник пропускается.
