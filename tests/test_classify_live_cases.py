...
# 24.09: у владельца появились готовые ответы (faq) — о себе, последний проект,
# город и формат, выход, платёжный опыт. Толку от них нет, если классификатор
# не узнаёт вопрос: всё неузнанное уходит карточкой. Здесь — частые формулировки,
# включая настоящий вопрос GDL IT, который месяц пролежал без ответа.
@pytest.mark.parametrize("text, label", [
    ("Какой формат работы рассматриваете?", "work_format"),
    ("Готовы к офису?", "work_format"),
    ("Готовы ли к посещению офиса в Москве?", "work_format"),
    ("Удалёнка подходит?", "work_format"),
    ("Where are you based? Is remote ok?", "work_format"),
    ("Есть опыт в финтехе?", "tech_question"),
    ("Расскажите о вашем последнем проекте, какие технологии там использовались?",
     "tech_question"),
    ("Когда сможете выйти?", "tech_question"),
    ("When can you start?", "tech_question"),
    # соседние правила не задеты
    ("Когда вам удобно созвониться?", "ask_call"),
    ("Пришлите свое резюме на рассмотрение, пожалуйста", "ask_cv"),
    ("Какие зарплатные ожидания?", "money"),
])
def test_questions_answered_from_owner_faq(text, label):
