...
from universal_content import MODULES as BASE_MODULES, MERGE_MAP, GLOSSARY
from v5_enrichment import prepare
from v5_pages import add_merge_intro, add_cards, add_client_page, zone_for, card_key, game_data
from v5_diagrams import add_diagrams
from universal_sources_extra import EXTRA, EXTRA_BY_MODULE
from execution_plan_content import add_execution_plan

ROOT=Path('C:/Projects/jobhunter')
TMP=ROOT/'tmp/pdfs'
OUT=ROOT/'output/pdf'
PDF=OUT/'AI_ML_Architect_Единый_план_Сурен_v5.pdf'
...
MODULES=deepcopy(BASE_MODULES)
EXTRA_BY_MODULE=deepcopy(EXTRA_BY_MODULE)
V5_SOURCES=prepare(MODULES,EXTRA_BY_MODULE)
SOURCES={}
...
class Book:
    def __init__(self,path,old=None,total=None):
        ...
        self.c.setTitle('AI/ML Architect - Универсальный план для Сурена, v5')
    ...
    def new(self,title,kicker='Универсальный маршрут',key=None,subtitle=None,outline=True):
        ...
        c.setFont('AR',8);c.setFillColor(MUTED);c.drawString(MARGIN,28,'Единый план v5  |  06.09.2026')
    ...
    def module(self,m):
        ...
        z=zone_for(code)
        if z:self.p(f'Зона {z["id"]}: {z["title"]}  |  {m["hours"]*10} XP после зачёта',9.1,True,color=TEAL,space=6)
        ...
        if z:self.p(self.bookmark_link(card_key(code),'Два вопроса колоды / '),9.0,markup=True,space=4)
...
def cover(b):
    ...
    c.setFillColor(HexColor('#7adbd2'));c.setFont('AB',10);c.drawString(MARGIN,H-55,'ЕДИНЫЙ ПЛАН  /  ВЕРСИЯ 5')
    ...
    c.setFillColor(HexColor('#bbced9'));c.setFont('AR',10);c.drawString(MARGIN,70,'Видео, схемы, 35 недель, 7 зон и 48 карточек повторения')
    c.setFont('AR',9);c.drawString(MARGIN,49,'Объединены учебный и игровой планы  |  6 сентября 2026')
...
def front(b):
    ...
    b.p('Это единая программа на основе нашего v4 и присланного игрового v4. Ранние версии уже включены. Темы, практика, видео, схемы и календарь согласованы; игровой формат добавляет контроль и повторение без двойного учёта часов.',11,space=13)
    ...
    b.section('План выполнения по неделям','После диагностики и нужной базы открой раздел «План выполнения»: 35 недель при 20 ч/неделю, задания, результаты, контрольные точки и четыре календарные недели пилота. Номера модулей ведут к материалам.')
    ...
    for part,mods in enumerate([MODULES[:16],MODULES[16:]]):
        ...
        if part==0:
            b.label('Календарь и выполнение')
            for key,title in [('merge_v5','Что объединено и как проходить'),('zone_map','Семь зон и контрольные испытания'),('cards_1','Колода из 48 вопросов'),('execution','План выполнения и ритм недели'),('execution_weeks_1','Недельные задания: 1-35'),('execution_gates','Контрольные точки'),('execution_variants','База, углубления и другой темп'),('execution_tracker','Карточка выполнения')]:
...
def atlas(b):
    ...
    b.table([['Точка потери ответа','Что проверить'],['Парсинг / OCR','Есть ли извлечённый текст и нужная страница.'],['Chunking','Не разделены ли таблица и заголовок; сохранена ли версия.'],['Embeddings / индекс','Совместимы ли представления документа и запроса.'],['Retrieval / ACL','Попал ли разрешённый источник в кандидаты; что отсёк фильтр.'],['Reranking / контекст','Не потерян ли найденный кандидат; отсутствующий нельзя восстановить ранжированием.'],['Генерация / ссылка','Поддерживает ли источник утверждение и верно ли указана страница.']],[CW*.34,CW*.66],size=9.3,pad=5)
...
def provenance(b):
    ...
    b.section('Основа объединения','Наш универсальный v4 (130 страниц) и AI_ML_Architect_План_v4_СуренCLODE.pdf (53 страницы). Ранние планы v2 на 48 и 34 страницы уже входили в универсальную версию. Сохранена связь с вакансией Интерика Лаб; таблицы соответствия находятся в начале.')
    b.section('Что изменено','В v5 объединены 35-недельный календарь и игровая карта: 7 зон, 21 испытание, 14 достижений, 48 вопросов с опорными ответами. Добавлены схемы доверия, outbox, размещения и каналов; уточнены упражнения. Сохранены исправления КПП, OData, НДС, гарантий внешней записи, handoff и обработки данных.')
...
def build(path,old=None,total=None):
    ...
    add_merge_intro(b,globals())
    allocations=add_execution_plan(b,globals())
    b.execution_allocations=allocations
    for m in MODULES:b.module(m)
    atlas(b);add_diagrams(b,globals());metrics_templates(b);checks_interview(b);add_client_page(b,globals());add_cards(b,globals());glossary_tracker(b);registry(b);provenance(b);b.save();return b

if __name__=='__main__':
    first=build(TMP/'universal_v5_firstpass.pdf')
    ...
    meta['execution_allocations']=book.execution_allocations
    meta['execution_weeks']=len(book.execution_allocations)
    meta['zones']=7;meta['boss_tests']=21;meta['flashcards']=48;meta['achievements']=14;meta['core_xp']=CORE*10
    (TMP/'universal_v5_build_meta.json').write_text(json.dumps(meta,ensure_ascii=False,indent=2),encoding='utf-8')
    (OUT/'assets/universal_v5_sources_registry.json').write_text(json.dumps({s:SOURCES[s] for s in USED},ensure_ascii=False,indent=2),encoding='utf-8')
    print(json.dumps({k:v for k,v in meta.items() if k not in ('page_map','bottom_positions','execution_allocations')},ensure_ascii=False,indent=2))
