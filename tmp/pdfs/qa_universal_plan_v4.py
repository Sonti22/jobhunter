...
meta=json.loads((P/'universal_v4_build_meta.json').read_text(encoding='utf-8'))
...
imgs=sorted(P.glob('universal_v4_render-*.png'))
...
for start in range(0,len(imgs),16):
    ...
    sh.save(P/f'universal_v4_contact_{start//16+1:02d}.jpg',quality=90)
...
(P/'universal_v4_qa_report.json').write_text(json.dumps(report,ensure_ascii=False,indent=2),encoding='utf-8')
