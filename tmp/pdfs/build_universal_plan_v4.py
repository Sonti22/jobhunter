...
TMP=ROOT/'tmp/pdfs'
OUT=ROOT/'output/pdf'
PDF=OUT/'AI_ML_Architect_Универсальный_план_Сурен_v4.pdf'
...
class Book:
    def __init__(self,path,old=None,total=None):
        ...
        self.c.setTitle('AI/ML Architect - Универсальный план для Сурена, v4')
    ...
    def new(self,title,kicker='Универсальный маршрут',key=None,subtitle=None,outline=True):
        ...
        c.setFont('AR',8);c.setFillColor(MUTED);c.drawString(MARGIN,28,'Универсальный план v4  |  06.09.2026')
...
def cover(b):
    ...
    c.setFillColor(HexColor('#7adbd2'));c.setFont('AB',10);c.drawString(MARGIN,H-55,'УНИВЕРСАЛЬНЫЙ ПЛАН  /  ВЕРСИЯ 4')
    ...
    c.setFillColor(HexColor('#bbced9'));c.setFont('AR',10);c.drawString(MARGIN,70,'Видео, схемы, практика и план выполнения по неделям')
    c.setFont('AR',9);c.drawString(MARGIN,49,'Добавлен календарь выполнения  |  6 сентября 2026')
...
def front(b):
    ...
    for part,mods in enumerate([MODULES[:16],MODULES[16:]]):
        ...
        if part==0:
            ...
            for key,title in [('execution','План выполнения и ритм недели'),('execution_weeks_1','Недельные задания: 1-35'),('execution_gates','Контрольные точки'),('execution_variants','База, углубления и другой темп'),('execution_tracker','Карточка выполнения')]:
...
def provenance(b):
    ...
    b.section('Что изменено','В v4 добавлен календарь: 35 недель основного маршрута, первая неделя по сессиям, контрольные точки, варианты темпа и личная карточка. Архитектура, discovery, CI и очередь перенесены к началу. Российские данные, диалоги, клиентская работа и интеграции сохранены. Финальный пилот проводится. Уточнены КПП, OData, НДС, handoff-метрики, стоимость ручного труда и восстановление после удаления.')
...
if __name__=='__main__':
    first=build(TMP/'universal_v4_firstpass.pdf')
    ...
    (TMP/'universal_v4_build_meta.json').write_text(json.dumps(meta,ensure_ascii=False,indent=2),encoding='utf-8')
